---
title: "Yanny-Laurel Analysis (Noah)"
output:
  html_document:
    df_print: paged
---
...
Let's get started. You should have downloaded the Yanny/Laurel demo repository from Github, but if not you can do so now via [this link](https://github.com/NoahMba/expra-demo). 
...
for (f in files) {
  ...
  cleaned <- incoming[!is.na(incoming[,1]),]  # filter
...
data_orig <- data_raw %>% dplyr::rename(music_yrs = years.musical.experience, # rename variables
                                    resp = colnames(data_raw)[1], # whether they responded yanny (2) or laurel (1)
...
hist(age_dat$age)
mean(age_dat$age) 
sd(age_dat$age) * ((length(age_dat$age) - 1) / length(age_dat$age))
# empirical SD

music_dat <- data %>% dplyr::group_by(participant) %>% summarise(music_yrs = unique(music_yrs))
mean(music_dat$music_yrs)
sd(music_dat$music_yrs) * (6/7)
hist(age_dat$age)
```
...
ggplot(data = prop_data, mapping = aes(dB_ratio, prop_yanny, color = participant)) +
  ...
  theme_classic()
ggsave(file.path(results_path, "fig1.png"), width = 6, height = 3)
...
prop_data_demo <- aggregate(resp ~ sex, data = data, FUN=mean)
...
prop_data_demo3 <- data %>% dplyr::group_by(age, dB_ratio) %>% 
                      dplyr::summarise(prop_yanny = mean(resp),
                                       sd = sd(resp),
                                       median_RT = median(RT),
                                       iqr_RT = IQR(RT),
...
ggplot(data = prop_data_demo3, mapping = aes(dB_ratio, prop_yanny, color = factor(age))) +
 geom_point() +
 geom_line(mapping = aes(group = factor(age))) +
 scale_y_continuous(name = "Prop. 'Yanny'", limits = c(0,1)) +
 scale_x_discrete(name = "High/low ratio (dB)", limits = unique(prop_data$dB_ratio)) + 
 theme_classic()

#Plotting the effect of musical experience
prop_data_demo4 <- data %>% dplyr::group_by(music_yrs, dB_ratio) %>% 
                      dplyr::summarise(prop_yanny = mean(resp),
                                       sd = sd(resp),
                                       median_RT = median(RT),
                                       iqr_RT = IQR(RT),
...
ggplot(data = prop_data_demo4, mapping = aes(dB_ratio, prop_yanny, color = factor(music_yrs))) +
 geom_point() +
 geom_line(mapping = aes(group = factor(music_yrs))) +
 scale_y_continuous(name = "Prop. 'Yanny'", limits = c(0,1)) +
 scale_x_discrete(name = "High/low ratio (dB)", limits = unique(prop_data$dB_ratio)) + 
 theme_classic()
...
We decided to remove the variable of sex. Can you recall the argument as to why? The direction of the effect goes against our hypothesis and it only explains a tiny amount of variance.
